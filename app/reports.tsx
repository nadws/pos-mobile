...
export default function ReportsScreen() {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get(
                `https://pos.soondobu.com/api/pos/${slug}/reports`,
    ...
    const chartData = {
        labels: reportData?.chart_labels?.length > 0 ? reportData.chart_labels : ["Mon", "Tue", "Wed"],
        datasets: [{
            data: reportData?.chart_values?.length > 0 ? reportData.chart_values : [0, 0, 0],
        ...
        <View style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerContent}>
                    ...
                    <Text style={styles.headerValue}>{formatRp(reportData?.weekly_revenue)}</Text>
                </View>
            </View>
                    ...
                    {reportData?.top_products?.map((item: any, index: number) => (
                        <View key={index} style={styles.rankRow}>
                            <View style={styles.rankInfo}>
                                <View style={[styles.rankDot, {backgroundColor: index === 0 ? '#FACC15' : '#2563EB'}]} />
...
const styles = StyleSheet.create({
    ...
    header: { backgroundColor: '#2563EB', paddingTop: 50, paddingBottom: 40, paddingHorizontal: 20, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
    ...
    headerValue: { color: 'white', fontSize: 28, fontWeight: 'bold', marginTop: 5 },
    ...
    statsCard: { flex: 1, backgroundColor: 'white', borderRadius: 20, padding: 16, alignItems: 'center', elevation: 2 },
    ...
    statsValue: { fontSize: 20, fontWeight: 'bold', color: '#111827', marginTop: 4 },
    rankRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
