...
const NOTIF_URL = 'https://assets.mixkit.co/active_storage/sfx/2358/2358-preview.mp3';
...
export default function KitchenScreen() {
    ...
    async function playNotificationSound() {
        try {
            const { sound } = await Audio.Sound.createAsync({ uri: NOTIF_URL }, { shouldPlay: true });
    ...
    const handleMarkItemReady = async (itemId: number) => {
        ...
        setOrders(prevOrders => prevOrders.map(order => ({
            ...order,
            items: order.items.map(item => item.id === itemId ? { ...item, status: 'done' } : item)
        })));
                ...
                >
                    {orders.length === 0 ? (
                        <View style={styles.emptyState}>
                            <MaterialCommunityIcons name="check-all" size={80} color="#D1D5DB" />
                        ...
                        orders.map((order) => {
                            ...
                            return (
                                <View key={order.id} style={styles.card}>
                                    ...
                                    <View style={[styles.cardTop, { borderLeftColor: getCardColor(mins) }]}>
                                        <View style={styles.cardInfo}>
                                            <View style={{flexDirection: 'row', alignItems: 'center', gap: 8}}>
                                                <Text style={styles.invoiceText}>#{order.id.toString().slice(-4)}</Text>
                                                {/* Badge Meja / Takeaway */}
                                                <View style={[styles.badge, order.table_number ? styles.badgeDineIn : styles.badgeTakeaway]}>
                                                    <MaterialCommunityIcons 
                                                        name={order.table_number ? "table-furniture" : "shopping"} 
                                                        size={12} 
                                                        color={order.table_number ? "#1E40AF" : "#991B1B"} 
                                                    />
                                                    <Text style={[styles.badgeText, { color: order.table_number ? "#1E40AF" : "#991B1B" }]}>
                                        ...
                                        {/* TIMER POJOK KANAN */}
                                        <View style={[styles.timerBadge, isLate && styles.timerLate]}>
                                            <MaterialCommunityIcons name="clock-outline" size={14} color={isLate ? "white" : "#4B5563"} />
                                            <Text style={[styles.timerText, isLate && { color: 'white' }]}>
                                                {mins} mnt
                                            </Text>
                                        </View>
                                    </View>
                                        ...
                                        {order.items.map((item, idx) => {
                                            ...
                                            return (
                                                <TouchableOpacity 
                                                    key={idx}
                                                    activeOpacity={0.7}
                                                    onPress={() => !isDone && handleMarkItemReady(item.id)}
                                                    style={[styles.itemRow, isDone && styles.itemRowDone]}
                                                >
                                                    {/* QTY BOX - FOKUS UTAMA */}
                                                    <View style={[styles.qtyBox, isDone ? styles.qtyDone : styles.qtyActive]}>
                                                        <Text style={[styles.qtyText, isDone && { color: '#9CA3AF' }]}>{item.quantity}</Text>
                                                    </View>

                                                    <View style={{flex: 1}}>
                                                        <Text style={[styles.productName, isDone && styles.textStrike]}>
                                                            {item.product?.name || item.product_name || 'Item dihapus'}
                                                        </Text>
                                                        {/* NOTES HIGHLIGHT */}
                                                        {item.note && (
                                                    ...
                                                    {/* CHECKBOX VISUAL */}
                                                    <View style={[styles.checkBox, isDone ? styles.checkDone : styles.checkPending]}>
                                                        {isDone && <MaterialCommunityIcons name="check" size={16} color="white" />}
                                                    </View>
                                                </TouchableOpacity>
                                            )
                                        })}
...
const styles = StyleSheet.create({
    ...
    // Header
    header: { 
        backgroundColor: '#1E40AF', padding: 16, paddingTop: 40, 
    ...
    customerName: { fontSize: 14, color: '#4B5563', marginTop: 2, fontWeight: '500' },
    
    badge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginLeft: 8, gap: 4 },
    badgeDineIn: { backgroundColor: '#DBEAFE' },
    badgeTakeaway: { backgroundColor: '#FEE2E2' },
    badgeText: { fontSize: 10, fontWeight: 'bold' },

    timerBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#E5E7EB', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, gap: 4 },
    ...
    qtyActive: { backgroundColor: '#EFF6FF', borderWidth: 1, borderColor: '#BFDBFE' },
    ...
    noteText: { fontSize: 12, color: '#B91C1C', fontWeight: '500', fontStyle: 'italic' },
