...
export default function PaymentScreen() {
    ...
    const loadData = async () => {
        try {
            ...
            if (savedCart) {
                ...
                setCart(parsedCart);
                const total = parsedCart.reduce((acc: number, item: CartItem) => acc + (item.price * item.qty), 0);
    ...
    const handleProcessPayment = async () => {
        ...
        try {
            ...
            const response = await axios.post(`https://pos.soondobu.com/api/pos/${slug}/checkout`, payload, {
                headers: { Authorization: `Bearer ${token}` }
            });

            if (response.data?.order?.invoice_number) setInvoiceNum(response.data.order.invoice_number);
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.card}>
                    <TouchableOpacity style={styles.summaryHeader} onPress={() => setShowSummary(!showSummary)}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                            <MaterialCommunityIcons name="receipt" size={22} color="#2563EB" />
                            <Text style={styles.cardTitle}>Detail Pesanan</Text>
                        </View>
                        <MaterialCommunityIcons name={showSummary ? "chevron-up" : "chevron-down"} size={24} color="#9CA3AF" />
                    </TouchableOpacity>
                    {showSummary && (
                        <View style={styles.summaryList}>
                            {cart.map((item, idx) => (
                                <View key={idx} style={styles.summaryItem}>
                                    ...
                                    <Text style={styles.itemPrice}>{formatRp(item.price * item.qty)}</Text>
                                </View>
                            ))}
                        </View>
                    )}
                ...
                <View style={styles.methodGrid}>
                    <TouchableOpacity 
                        ...
                        onPress={() => { setPaymentMethod('cash'); setCashGiven(''); }}
                    >
                        <MaterialCommunityIcons name="cash" size={28} color={paymentMethod === 'cash' ? "#2563EB" : "#9CA3AF"} />
                        <Text style={[styles.methodText, paymentMethod === 'cash' && styles.methodTextActive]}>Tunai</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                        style={[styles.methodCard, paymentMethod === 'qris' && styles.methodActive]} 
                        onPress={() => setPaymentMethod('qris')}
                    >
                        <MaterialCommunityIcons name="qrcode-scan" size={24} color={paymentMethod === 'qris' ? "#2563EB" : "#9CA3AF"} />
                    ...
                    <View style={styles.cashSection}>
                        ...
                        <View style={styles.quickGrid}>
                            {[50000, 100000].map((amt) => (
                                <TouchableOpacity key={amt} style={styles.quickBtn} onPress={() => setCashGiven(amt.toLocaleString("id-ID"))}>
                                    <Text style={styles.quickBtnText}>{amt / 1000}k</Text>
                                </TouchableOpacity>
                            ))}
                            <TouchableOpacity style={styles.uangPasBtn} onPress={() => setCashGiven(totalAmount.toLocaleString("id-ID"))}>
                                <Text style={styles.uangPasText}>Uang Pas</Text>
                            </TouchableOpacity>
                        </View>

                        <View style={[styles.changeBox, change < 0 ? styles.changeRed : styles.changeGreen]}>
                            <Text style={styles.changeLabel}>{change < 0 ? "Kurang" : "Kembalian"}</Text>
                            <Text style={styles.changeValue}>{formatRp(Math.abs(change))}</Text>
                        </View>
                    </View>
                )}
            </ScrollView>
            ...
            <View style={styles.footer}>
                <TouchableOpacity
                    style={[styles.payButton, (isProcessing || (paymentMethod === 'cash' && change < 0)) && styles.payButtonDisabled]}
                    onPress={handleProcessPayment}
                    disabled={isProcessing || (paymentMethod === 'cash' && change < 0)}
                >
                    {isProcessing ? <ActivityIndicator color="white" /> : <Text style={styles.payButtonText}>PROSES PEMBAYARAN</Text>}
                </TouchableOpacity>
            </View>
            ...
            <Modal visible={successModal} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.successCard}>
                        ...
                        <TouchableOpacity style={styles.finishBtn} onPress={() => { setSuccessModal(false); router.replace('/pos'); }}>
...
const styles = StyleSheet.create({
    ...
    card: { backgroundColor: 'white', borderRadius: 20, padding: 16, marginBottom: 20, elevation: 2, shadowColor: '#000', shadowOpacity: 0.05 },
    summaryHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    cardTitle: { fontSize: 16, fontWeight: 'bold', color: '#1F2937' },
    summaryList: { marginTop: 15, borderTopWidth: 1, borderTopColor: '#F3F4F6', paddingTop: 10 },
    ...
    sectionTitle: { fontSize: 15, fontWeight: 'bold', color: '#1F2937', marginBottom: 12, marginLeft: 5 },
    methodGrid: { flexDirection: 'row', gap: 12, marginBottom: 25 },
    methodCard: { flex: 1, height: 90, backgroundColor: 'white', borderRadius: 16, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: 'transparent', elevation: 1 },
    methodActive: { borderColor: '#2563EB', backgroundColor: '#EFF6FF' },
    methodText: { marginTop: 8, fontSize: 14, fontWeight: '600', color: '#9CA3AF' },
    methodTextActive: { color: '#2563EB' },

    cashSection: { marginTop: 5 },
    labelInput: { fontSize: 13, color: '#6B7280', marginBottom: 8, marginLeft: 5 },
    inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', borderRadius: 16, paddingHorizontal: 18, height: 65, elevation: 1 },
    prefix: { fontSize: 20, fontWeight: 'bold', color: '#9CA3AF', marginRight: 8 },
    input: { flex: 1, fontSize: 24, fontWeight: 'bold', color: '#1F2937' },

    quickGrid: { flexDirection: 'row', gap: 10, marginTop: 15 },
    quickBtn: { flex: 1, backgroundColor: 'white', paddingVertical: 12, borderRadius: 12, alignItems: 'center', elevation: 1 },
    quickBtnText: { fontWeight: 'bold', color: '#4B5563' },
    uangPasBtn: { flex: 2, backgroundColor: '#DBEAFE', paddingVertical: 12, borderRadius: 12, alignItems: 'center' },
    uangPasText: { color: '#2563EB', fontWeight: 'bold' },

    changeBox: { flexDirection: 'row', justifyContent: 'space-between', padding: 18, borderRadius: 16, marginTop: 20 },
    ...
    payButton: { backgroundColor: '#2563EB', padding: 18, borderRadius: 16, alignItems: 'center' },
    payButtonDisabled: { backgroundColor: '#9CA3AF' },
    payButtonText: { color: 'white', fontWeight: 'bold', fontSize: 16 },

    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 },
    successCard: { backgroundColor: 'white', width: '100%', borderRadius: 24, padding: 30, alignItems: 'center' },
    successTitle: { fontSize: 24, fontWeight: 'bold', color: '#111827', marginTop: 10 },
    successDivider: { width: '100%', height: 1, backgroundColor: '#E5E7EB', marginVertical: 20 },
    printBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 15, width: '100%', borderRadius: 12, borderWidth: 1, borderColor: '#2563EB', justifyContent: 'center', marginBottom: 12 },
    printText: { color: '#2563EB', fontWeight: 'bold' },
    finishBtn: { backgroundColor: '#2563EB', paddingVertical: 15, width: '100%', borderRadius: 12, alignItems: 'center' },
