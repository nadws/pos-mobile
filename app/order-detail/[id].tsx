      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.itemsSection}>
            ...
            <View key={index} style={styles.itemRow}>
              <View style={styles.itemInfo}>
                <Text style={styles.itemName}>
                  {item.product?.name || item.product_name || "Menu"}
                </Text>
                <Text style={styles.itemSub}>
                  {item.quantity || item.qty} x {formatRp(item.price)}
                </Text>
              </View>
              <Text style={styles.itemTotal}>
                {formatRp((item.quantity || item.qty) * item.price)}
              </Text>
            </View>
          ))}
          ...
          <Text style={styles.paymentMethod}>
            Metode: {order.payment_method?.toUpperCase() || "CASH"}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    padding: 20,
    paddingBottom: 40, // bukan marginBottom
  },
