...
export default function KitchenScreen() {
    ...
    useEffect(() => {
        ...
        intervalRef.current = setInterval(() => { fetchOrders(true); }, 7000);
            ...
            <View style={styles.bodyContainer}>
                {loading && !refreshing ? (
                    ...
                    <ScrollView 
                        ...
                        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchOrders()} tintColor="#2563EB" />}
                    >
                        {orders.length === 0 ? (
                            <View style={styles.emptyCard}>
                                <MaterialCommunityIcons name="chef-hat" size={60} color="#E5E7EB" />
                            ...
                            orders.map((order) => (
                                <View key={order.id} style={styles.card}>
                                    <View style={styles.cardHeader}>
                                        <View style={{flex: 1}}>
                                            <Text style={styles.customerName}>{order.customer_name || 'Pelanggan'}</Text>
                                            <Text style={styles.timeText}>#{order.id} • {formatTime(order.created_at)}</Text>
                                        </View>
                                        <View style={[styles.typeBadge, {backgroundColor: order.table_number ? '#DBEAFE' : '#FEE2E2'}]}>
                                            <Text style={[styles.typeText, {color: order.table_number ? '#2563EB' : '#EF4444'}]}>
                                    ...
                                    {order.items.map((item) => (
                                        <TouchableOpacity 
                                            ...
                                            onPress={() => item.status !== 'done' && handleMarkItemReady(item.id)}
                                        >
                                            <View style={[styles.checkBox, item.status === 'done' ? styles.checkActive : styles.checkInactive]}>
                                                {item.status === 'done' && <MaterialCommunityIcons name="check" size={12} color="white" />}
                                            </View>
                                            <View style={{flex: 1}}>
                                                <Text style={[styles.itemText, item.status === 'done' && styles.textStrike]}>
                                                    <Text style={{fontWeight: 'bold', color: '#2563EB'}}>{item.quantity}x </Text> 
                                                    {item.product?.name || item.product_name}
                                                </Text>
                                                {item.note && <Text style={styles.noteText}>Ket: {item.note}</Text>}
                                            </View>
                                        </TouchableOpacity>
                                    ))}
                                </View>
...
const styles = StyleSheet.create({
    ...
    headerContainer: { backgroundColor: '#2563EB', paddingTop: 60, paddingBottom: 40, paddingHorizontal: 20, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
    ...
    card: { backgroundColor: 'white', borderRadius: 20, padding: 16, marginBottom: 15, elevation: 3 },
    ...
    itemRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 12 },
    checkBox: { width: 24, height: 24, borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
    ...
    emptyCard: { backgroundColor: 'white', padding: 40, borderRadius: 20, alignItems: 'center' },
